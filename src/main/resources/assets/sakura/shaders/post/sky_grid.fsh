...
void main() {
    float iTime = Params2.x;
    vec2 iResolution = max(InSize.xy, vec2(1.0));
    float alpha = clamp(Params1.x, 0.0, 1.0);
    float yaw = Params3.x;
    float pitch = Params3.y;
    ...
    if (uv.y < -0.2) {
        vec2 uv2 = uv;
        uv2.y = 3.0 / (abs(uv2.y + 0.2) + 0.05);
        uv2.x *= uv2.y * 1.0;
        float gridVal = grid(uv2, battery, iTime);
        col = mix(col, vec3(1.0, 0.5, 1.0), gridVal);
    } else {
        float fujiD = min(uv.y * 4.5 - 0.5, 1.0);
        vec2 uv2 = uv;
        uv2.y -= battery * 1.1 - 0.51;

        vec2 sunUV = uv2 + vec2(0.75, 0.2);
        ...
        float fujiVal = sdTrapezoid(uv2 + vec2(-0.75 + sunUV.y * 0.0, 0.5), 1.75 + pow(uv2.y * uv2.y, 2.1), 0.2, 0.5);
        float waveVal = uv2.y + sin(uv2.x * 20.0 + iTime * 2.0) * 0.05 + 0.2;
        ...
        col += mix(col, mix(vec3(1.0, 0.12, 0.8), vec3(0.0, 0.0, 0.2), clamp(uv2.y * 3.5 + 3.0, 0.0, 1.0)), step(0.0, fujiVal));
