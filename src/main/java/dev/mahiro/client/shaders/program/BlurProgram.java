package dev.mahiro.client.shaders.program;
...
public class BlurProgram {
    ...
    private static final Identifier FRAGMENT_SHADER = Identifier.of("mahiro", "core/blur");
    ...
    private void ensureProgram() {
        if (this.uniforms == null) {
            this.uniforms = new MappableRingBuffer(() -> "Mahiro BlurUniforms", GpuBuffer.USAGE_MAP_WRITE | GpuBuffer.USAGE_UNIFORM, UNIFORMS_SIZE);
        }
        if (this.pipeline == null) {
            this.pipeline = RenderPipeline.builder(RenderPipelines.POST_EFFECT_PROCESSOR_SNIPPET)
                    .withLocation(Identifier.of("mahiro", "pipeline/blur"))
    ...
    private void ensureInputFramebuffer(int width, int height) {
        if (this.input == null) {
            this.input = new SimpleFramebuffer("Mahiro Blur Input", width, height, false);
            return;
        }
    ...
    public void renderRoundedBlur(float x, float y, float width, float height, float radius, Color color, float blurStrength, float blurOpacity) {
        ...
        try (RenderPass renderPass = encoder.createRenderPass(() -> "Mahiro Blur", framebuffer.getColorAttachmentView(), OptionalInt.empty(), framebuffer.useDepthAttachment ? framebuffer.getDepthAttachmentView() : null, OptionalDouble.empty())) {
