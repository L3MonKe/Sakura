...
public class Chams extends Module {
    ...
    public static final RenderPipeline SAKURA_ENTITY_CHAMS_PIPELINE = RenderPipelines.register(RenderPipeline.builder(RenderPipelines.ENTITY_SNIPPET).withLocation("pipeline/sakura_entity_chams").withShaderDefine("ALPHA_CUTOUT", 0.1f).withShaderDefine("PER_FACE_LIGHTING").withSampler("Sampler1").withBlend(BlendFunction.TRANSLUCENT).withCull(false).withDepthBias(-1.0f, -1100000.0f).build());
    private static final Function<Identifier, RenderLayer> SAKURA_ENTITY_CHAMS_LAYER = Util.memoize(texture -> RenderLayer.of("sakura_entity_chams", RenderSetup.builder(SAKURA_ENTITY_CHAMS_PIPELINE).texture("Sampler0", texture).useLightmap().useOverlay().crumbling().translucent().outlineMode(RenderSetup.OutlineMode.AFFECTS_OUTLINE).build()));

    public static RenderLayer getChamsLayer(Identifier texture) {
        return SAKURA_ENTITY_CHAMS_LAYER.apply(texture);
    }
