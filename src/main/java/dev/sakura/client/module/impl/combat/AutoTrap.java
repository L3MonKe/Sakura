...
public class AutoTrap extends Module {
    ...
    private final BoolValue legs = new BoolValue("Legs", "腿部", false);
    ...
    public AutoTrap() {
        super("AutoTrap", "自动困住目标", Category.Combat);
    }
    ...
    private void onTick(TickEvent.Pre event) {
        ...
        PlayerEntity target = getTarget(range.get());
        if (target == null) {
            surroundPositions.clear();
            return;
        }
    ...
    private PlayerEntity getTarget(double range) {
        if (mc.world == null || mc.player == null) return null;
        return StreamSupport.stream(mc.world.getEntities().spliterator(), false)
                .filter(e -> e instanceof PlayerEntity && e != mc.player && !e.isSpectator() && e.isAlive())
