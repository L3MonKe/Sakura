...
public class Animations extends Module {
    public Animations() {
        super("Animations", "自定义动画", Category.Render);
        this.setType(ModuleType.All);
    }

    public final EnumValue<SwingMode> swingMode = new EnumValue<>("SwingMode", "挥动模式", SwingMode.Default);

    public final BoolValue slowAnimation = new BoolValue("SlowAnimation", "缓慢动画", true);
    public final NumberValue<Integer> slowAnimationVal = new NumberValue<>("SlowValue", "缓慢速度", 12, 1, 50, 1);
    ...
    public void renderFirstPersonItemCustom(AbstractClientPlayerEntity player, float tickDelta, float pitch, Hand hand, float swingProgress, ItemStack item, float equipProgress, MatrixStack matrices, VertexConsumerProvider vertexConsumers, int light) {
        if (!player.isUsingSpyglass()) {
            ...
            if (item.isOf(Items.CROSSBOW)) {
                ...
                boolean isFake = item.getItem() instanceof net.minecraft.item.SwordItem && mc.options.useKey.isPressed() && hand == Hand.MAIN_HAND;
