/*
package com.lemonclient.client.command.impl;

import com.mojang.brigadier.arguments.StringArgumentType;
import com.mojang.brigadier.builder.LiteralArgumentBuilder;
import client.dev.lemonclient.Zeta;
import command.client.dev.lemonclient.Command;
import client.events.client.dev.lemonclient.ChatMessageEvent;
import client.lemonchat.dev.lemonclient.ChatClient;
...
public class IRCCommand extends Command {
    ...
    public void buildCommand(LiteralArgumentBuilder<CommandSource> literalArgumentBuilder) {
        literalArgumentBuilder.then(literal("bind").executes((context -> {
                    Zeta.EVENT_BUS.subscribe(handler);
                    return SINGLE_SUCCESS;
                }))
        );

        literalArgumentBuilder.then(literal("unbind").executes((context -> {
                    Zeta.EVENT_BUS.unsubscribe(handler);
                    return SINGLE_SUCCESS;
                }))
        );
