...
import com.zeta.client.account.msa.MSAAuthenticator;
import com.zeta.client.account.type.MinecraftAccount;
import com.zeta.client.mixin.accessor.IMinecraftClient;
...
public class AccountManager {
    ...
    public void register(MinecraftAccount account, boolean save) {
        for (MinecraftAccount existing : accounts) {
            if (existing.username().equalsIgnoreCase(account.username()) && existing.getClass() == account.getClass()) {
                Zeta.LOGGER.warn("Account already exists: {} ({})", account.username(), account.getClass().getSimpleName());
                return;
            }
        }
        accounts.add(account);
        if (save) Zeta.CONFIG.saveAccounts();
    }

    public void unregister(final MinecraftAccount account) {
        accounts.remove(account);
        Zeta.CONFIG.saveAccounts();
    }

    public void setSession(final Session session) {
        ((IMinecraftClient) mc).setSession(session);
        Zeta.LOGGER.info("Set session to {} ({})", session.getUsername(), session.getUuidOrNull());
    }
    ...
    public boolean isEncrypted() {
        return Zeta.CONFIG.isEncrypted();
    }
}
