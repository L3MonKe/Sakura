    ...
    public void buildCommand(LiteralArgumentBuilder<CommandSource> builder) {
        builder.then(argument("prefix", StringArgumentType.greedyString())
                        .executes(c -> {
                            ...
                            Zeta.COMMAND.setPrefix(newPrefix, prefixKey);
                            Zeta.CONFIG.savePrefix(newPrefix);
                            ChatUtil.addChatMessage("Command prefix set to: §a" + newPrefix);
                            return 1;
                        }))
                .executes(c -> {
                    ChatUtil.addChatMessage("Current prefix: §a" + Zeta.COMMAND.getPrefix());
                    ChatUtil.addChatMessage("Usage: " + Zeta.COMMAND.getPrefix() + "prefix <new prefix>");
                    return 1;
                });
    }
