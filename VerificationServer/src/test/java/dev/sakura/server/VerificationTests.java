...
public class VerificationTests {
    ...
    public void protocolRoundtrip() {
        ...
        LoginResultS2C in = new LoginResultS2C(true, 123L, 456L, "OK");
        byte[] encoded = protocol.encode(in);
        IRCPacket decoded = protocol.decodePayload(encoded);
        Assertions.assertInstanceOf(LoginResultS2C.class, decoded);
        LoginResultS2C out = (LoginResultS2C) decoded;
    ...
    public void handshakeIsAllowedWhenUserNull() {
        ...
        boolean allowed = handlerManager.allowNull(new HandshakeC2S("u", "t"));
        Assertions.assertTrue(allowed);
    }

    @Test
    public void authFlowRegisterLoginRecharge() throws Exception {
        Path dbFile = Files.createTempFile("impl", ".sqlite");
