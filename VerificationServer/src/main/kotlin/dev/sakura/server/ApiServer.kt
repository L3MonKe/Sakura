...
private fun createServer(state: AppState): HttpServer {
    ...
    server.createContext("/api/v1/license/verify", JsonHandler(state) { ex, body ->
        if (ex.requestMethod != "POST") return@JsonHandler respondJson(ex, 405, mapOf("error" to "METHOD_NOT_ALLOWED"))
        val req = JSON.decodeFromString(EncryptedRequest.serializer(), body)
        val resp = handleLicenseVerify(state, req)
        respondJson(ex, 200, resp)
    })
    ...
    server.createContext("/api/v1/device/bind", JsonHandler(state) { ex, body ->
        if (ex.requestMethod != "POST") return@JsonHandler respondJson(ex, 405, mapOf("error" to "METHOD_NOT_ALLOWED"))
        val req = JSON.decodeFromString(EncryptedRequest.serializer(), body)
        val resp = handleDeviceBind(state, req)
        respondJson(ex, 200, resp)
    })

    server.createContext("/api/v1/device/unbind", JsonHandler(state) { ex, body ->
        if (ex.requestMethod != "POST") return@JsonHandler respondJson(ex, 405, mapOf("error" to "METHOD_NOT_ALLOWED"))
        val req = JSON.decodeFromString(EncryptedRequest.serializer(), body)
        val resp = handleDeviceUnbind(state, req)
        respondJson(ex, 200, resp)
    })

    server.createContext("/api/v1/device/rebind", JsonHandler(state) { ex, body ->
        if (ex.requestMethod != "POST") return@JsonHandler respondJson(ex, 405, mapOf("error" to "METHOD_NOT_ALLOWED"))
        val req = JSON.decodeFromString(EncryptedRequest.serializer(), body)
        val resp = handleDeviceRebind(state, req)
        respondJson(ex, 200, resp)
    })
...
private fun handleLicenseVerify(state: AppState, req: EncryptedRequest): EncryptedResponse {
    val session = requireSession(state, req.sid)
    val aad = ("sid=" + req.sid).toByteArray(StandardCharsets.UTF_8)
    val pt = aesGcmDecrypt(session.key32, B64D.decode(req.iv), B64D.decode(req.ct), aad)
    val plain = JSON.decodeFromString(LicenseVerifyPlainRequest.serializer(), pt.toString(StandardCharsets.UTF_8))

    if (!timestampOk(plain.ts)) return encryptUserAuthResponse(
        session,
        UserAuthPlainResponse(ok = false, error = "BAD_TIMESTAMP")
    )

    val key = normalizeLicenseKey(plain.licenseKey) ?: return encryptUserAuthResponse(
        session,
        UserAuthPlainResponse(ok = false, error = "BAD_REQUEST")
    )
    val deviceId = plain.deviceId.trim()
    if (key.isBlank() || deviceId.isBlank()) return encryptUserAuthResponse(
        session,
        UserAuthPlainResponse(ok = false, error = "BAD_REQUEST")
    )

    val lic0 = state.store.findLicenseByKey(key) ?: return encryptUserAuthResponse(
        session,
        UserAuthPlainResponse(ok = false, error = "LICENSE_NOT_FOUND")
    )
    if (lic0.status == "BANNED" || lic0.status == "REVOKED") return encryptUserAuthResponse(
        session,
        UserAuthPlainResponse(ok = false, error = "LICENSE_REVOKED")
    )
    ...
    if (plain.register) {
        ...
        val user = if (existingUser != null) {
            if (!state.store.verifyUserPassword(existingUser, password)) {
                return encryptUserAuthResponse(session, UserAuthPlainResponse(ok = false, error = "BAD_CREDENTIALS"))
            }
            existingUser
        } else {
            if (lic0.userId != null) {
                return encryptUserAuthResponse(session, UserAuthPlainResponse(ok = false, error = "LICENSE_ALREADY_CLAIMED"))
            }
            try {
                state.store.createUserAndClaimLicense(username, password, iterations = 200_000, licenseKey = key)
            } catch (e: Exception) {
                val msg = e.message ?: ""
                return encryptUserAuthResponse(
                    session,
        ...
        when (val ownerId = (state.store.findLicenseByKey(key) ?: lic0).userId) {
            null -> {
                val claimed = state.store.claimLicense(key, user.id)
                if (!claimed) return encryptUserAuthResponse(
                    session,
                    UserAuthPlainResponse(ok = false, error = "LICENSE_ALREADY_CLAIMED")
                )
            }
        ...
        val ownerId = lic0.userId
        if (ownerId != null && plain.username != null && plain.password != null) {
            val username = normalizeUsername(plain.username) ?: return encryptUserAuthResponse(
                session,
                UserAuthPlainResponse(ok = false, error = "BAD_REQUEST")
            )
            val user = state.store.findUserByUsername(username) ?: return encryptUserAuthResponse(
                session,
                UserAuthPlainResponse(ok = false, error = "USER_NOT_FOUND")
            )
            if (!state.store.verifyUserPassword(user, plain.password)) return encryptUserAuthResponse(
                session,
                UserAuthPlainResponse(ok = false, error = "BAD_CREDENTIALS")
            )
            if (user.id != ownerId) return encryptUserAuthResponse(
                session,
                UserAuthPlainResponse(ok = false, error = "LICENSE_NOT_OWNED")
            )
        }
    }

    val now = Instant.now().epochSecond
    val lic = state.store.activateIfNeeded(state.store.findLicenseByKey(key) ?: lic0, now)
    val exp = lic.expiresAt ?: 0L
    if (exp > 0 && now >= exp) return encryptUserAuthResponse(
        session,
        UserAuthPlainResponse(ok = false, error = "LICENSE_EXPIRED", licenseKey = key, expiresAt = exp)
    )

    val normDev = normalizeDeviceId(deviceId)
    val binding = state.store.getBinding(key)
    when {
        binding == null -> state.store.upsertBinding(key, normDev, iterations = 200_000)
        !state.store.verifyBinding(binding, normDev) -> return encryptUserAuthResponse(
            session,
            UserAuthPlainResponse(ok = false, error = "DEVICE_MISMATCH", licenseKey = key, expiresAt = exp)
        )
    }

    val token = signToken(state.signing.privateKey, key, exp)
    return encryptUserAuthResponse(
        session,
        UserAuthPlainResponse(ok = true, token = token, licenseKey = key, expiresAt = exp)
    )
}
...
private fun handleDeviceBind(state: AppState, req: EncryptedRequest): EncryptedResponse {
    val session = requireSession(state, req.sid)
    val aad = ("sid=" + req.sid).toByteArray(StandardCharsets.UTF_8)
    val pt = aesGcmDecrypt(session.key32, B64D.decode(req.iv), B64D.decode(req.ct), aad)
    val plain = JSON.decodeFromString(DeviceBindPlainRequest.serializer(), pt.toString(StandardCharsets.UTF_8))

    if (!timestampOk(plain.ts)) return encryptBindResponse(
        session,
        DeviceBindPlainResponse(ok = false, error = "BAD_TIMESTAMP")
    )

    val key = normalizeLicenseKey(plain.licenseKey) ?: return encryptBindResponse(
        session,
        DeviceBindPlainResponse(ok = false, error = "BAD_REQUEST")
    )
    val deviceId = plain.deviceId.trim()
    if (key.isBlank() || deviceId.isBlank()) return encryptBindResponse(
        session,
        DeviceBindPlainResponse(ok = false, error = "BAD_REQUEST")
    )

    val lic0 = state.store.findLicenseByKey(key) ?: return encryptBindResponse(
        session,
        DeviceBindPlainResponse(ok = false, error = "LICENSE_NOT_FOUND")
    )
    if (lic0.status == "BANNED" || lic0.status == "REVOKED") return encryptBindResponse(
        session,
        DeviceBindPlainResponse(ok = false, error = "LICENSE_REVOKED")
    )

    val now = Instant.now().epochSecond
    val lic = state.store.activateIfNeeded(lic0, now)
    val exp = lic.expiresAt ?: 0L
    if (exp > 0 && now >= exp) return encryptBindResponse(
        session,
        DeviceBindPlainResponse(ok = false, error = "LICENSE_EXPIRED", expiresAt = exp)
    )

    val ownerId = lic.userId
    if (ownerId != null) {
        val token = plain.token?.trim()?.takeIf { it.isNotBlank() }
        if (token != null) {
            val (payloadObj, tokenErr) = verifyTokenPayload(state.signing.publicKey, token)
            if (payloadObj == null) return encryptBindResponse(
                session,
                DeviceBindPlainResponse(ok = false, error = tokenErr ?: "INVALID_TOKEN", expiresAt = exp)
            )
            if (payloadObj.licenseKey != key) return encryptBindResponse(
                session,
                DeviceBindPlainResponse(ok = false, error = "TOKEN_LICENSE_MISMATCH", expiresAt = exp)
            )
        } else {
            val username = normalizeUsername(plain.username) ?: return encryptBindResponse(
                session,
                DeviceBindPlainResponse(ok = false, error = "CREDENTIALS_REQUIRED", expiresAt = exp)
            )
            val password = plain.password?.takeIf { it.isNotBlank() } ?: return encryptBindResponse(
                session,
                DeviceBindPlainResponse(ok = false, error = "CREDENTIALS_REQUIRED", expiresAt = exp)
            )
            val user = state.store.findUserByUsername(username) ?: return encryptBindResponse(
                session,
                DeviceBindPlainResponse(ok = false, error = "USER_NOT_FOUND", expiresAt = exp)
            )
            if (!state.store.verifyUserPassword(user, password)) return encryptBindResponse(
                session,
                DeviceBindPlainResponse(ok = false, error = "BAD_CREDENTIALS", expiresAt = exp)
            )
            if (user.id != ownerId) return encryptBindResponse(
                session,
                DeviceBindPlainResponse(ok = false, error = "LICENSE_NOT_OWNED", expiresAt = exp)
            )
        }
    }
    ...
    when {
        binding == null -> state.store.upsertBinding(key, normDev, iterations = 200_000)
        !state.store.verifyBinding(binding, normDev) -> return encryptBindResponse(
            session,
            DeviceBindPlainResponse(ok = false, error = "DEVICE_MISMATCH", expiresAt = exp)
        )
    }
    return encryptBindResponse(session, DeviceBindPlainResponse(ok = true, expiresAt = exp))
}

private fun handleDeviceUnbind(state: AppState, req: EncryptedRequest): EncryptedResponse {
    val session = requireSession(state, req.sid)
    val aad = ("sid=" + req.sid).toByteArray(StandardCharsets.UTF_8)
    val pt = aesGcmDecrypt(session.key32, B64D.decode(req.iv), B64D.decode(req.ct), aad)
    val plain = JSON.decodeFromString(DeviceUnbindPlainRequest.serializer(), pt.toString(StandardCharsets.UTF_8))

    if (!timestampOk(plain.ts)) return encryptUnbindResponse(
        session,
        DeviceUnbindPlainResponse(ok = false, error = "BAD_TIMESTAMP")
    )

    val key = normalizeLicenseKey(plain.licenseKey) ?: return encryptUnbindResponse(
        session,
        DeviceUnbindPlainResponse(ok = false, error = "BAD_REQUEST")
    )
    val deviceId = plain.deviceId.trim()
    if (key.isBlank() || deviceId.isBlank()) return encryptUnbindResponse(
        session,
        DeviceUnbindPlainResponse(ok = false, error = "BAD_REQUEST")
    )

    val lic0 = state.store.findLicenseByKey(key) ?: return encryptUnbindResponse(
        session,
        DeviceUnbindPlainResponse(ok = false, error = "LICENSE_NOT_FOUND")
    )
    if (lic0.status == "BANNED" || lic0.status == "REVOKED") return encryptUnbindResponse(
        session,
        DeviceUnbindPlainResponse(ok = false, error = "LICENSE_REVOKED")
    )

    val now = Instant.now().epochSecond
    val lic = state.store.activateIfNeeded(lic0, now)
    val exp = lic.expiresAt ?: 0L
    if (exp > 0 && now >= exp) return encryptUnbindResponse(
        session,
        DeviceUnbindPlainResponse(ok = false, error = "LICENSE_EXPIRED", expiresAt = exp)
    )

    val binding = state.store.getBinding(key) ?: return encryptUnbindResponse(
        session,
        DeviceUnbindPlainResponse(ok = false, error = "DEVICE_NOT_BOUND", expiresAt = exp)
    )
    val normDev = normalizeDeviceId(deviceId)
    ...
    if (token != null) {
        val (payloadObj, tokenErr) = verifyTokenPayload(state.signing.publicKey, token)
        if (payloadObj == null) return encryptUnbindResponse(
            session,
            DeviceUnbindPlainResponse(ok = false, error = tokenErr ?: "INVALID_TOKEN", expiresAt = exp)
        )
        if (payloadObj.licenseKey != key) return encryptUnbindResponse(
            session,
            DeviceUnbindPlainResponse(ok = false, error = "TOKEN_LICENSE_MISMATCH", expiresAt = exp)
        )
        if (!state.store.verifyBinding(binding, normDev)) return encryptUnbindResponse(
            session,
            DeviceUnbindPlainResponse(ok = false, error = "DEVICE_MISMATCH", expiresAt = exp)
        )
    } else {
        val ownerId = lic.userId ?: return encryptUnbindResponse(
            session,
            DeviceUnbindPlainResponse(ok = false, error = "AUTH_REQUIRED", expiresAt = exp)
        )
        val username = normalizeUsername(plain.username) ?: return encryptUnbindResponse(
            session,
            DeviceUnbindPlainResponse(ok = false, error = "CREDENTIALS_REQUIRED", expiresAt = exp)
        )
        val password = plain.password?.takeIf { it.isNotBlank() } ?: return encryptUnbindResponse(
            session,
            DeviceUnbindPlainResponse(ok = false, error = "CREDENTIALS_REQUIRED", expiresAt = exp)
        )
        val user = state.store.findUserByUsername(username) ?: return encryptUnbindResponse(
            session,
            DeviceUnbindPlainResponse(ok = false, error = "USER_NOT_FOUND", expiresAt = exp)
        )
        if (!state.store.verifyUserPassword(user, password)) return encryptUnbindResponse(
            session,
            DeviceUnbindPlainResponse(ok = false, error = "BAD_CREDENTIALS", expiresAt = exp)
        )
        if (user.id != ownerId) return encryptUnbindResponse(
            session,
            DeviceUnbindPlainResponse(ok = false, error = "LICENSE_NOT_OWNED", expiresAt = exp)
        )
    }

    state.store.deleteBinding(key)
    return encryptUnbindResponse(session, DeviceUnbindPlainResponse(ok = true, expiresAt = exp))
}

private fun handleDeviceRebind(state: AppState, req: EncryptedRequest): EncryptedResponse {
    val session = requireSession(state, req.sid)
    val aad = ("sid=" + req.sid).toByteArray(StandardCharsets.UTF_8)
    val pt = aesGcmDecrypt(session.key32, B64D.decode(req.iv), B64D.decode(req.ct), aad)
    val plain = JSON.decodeFromString(DeviceRebindPlainRequest.serializer(), pt.toString(StandardCharsets.UTF_8))

    if (!timestampOk(plain.ts)) return encryptRebindResponse(
        session,
        DeviceRebindPlainResponse(ok = false, error = "BAD_TIMESTAMP")
    )

    val key = normalizeLicenseKey(plain.licenseKey) ?: return encryptRebindResponse(
        session,
        DeviceRebindPlainResponse(ok = false, error = "BAD_REQUEST")
    )
    val newDeviceId = plain.newDeviceId.trim()
    if (key.isBlank() || newDeviceId.isBlank()) return encryptRebindResponse(
        session,
        DeviceRebindPlainResponse(ok = false, error = "BAD_REQUEST")
    )

    val lic0 = state.store.findLicenseByKey(key) ?: return encryptRebindResponse(
        session,
        DeviceRebindPlainResponse(ok = false, error = "LICENSE_NOT_FOUND")
    )
    if (lic0.status == "BANNED" || lic0.status == "REVOKED") return encryptRebindResponse(
        session,
        DeviceRebindPlainResponse(ok = false, error = "LICENSE_REVOKED")
    )

    val now = Instant.now().epochSecond
    val lic = state.store.activateIfNeeded(lic0, now)
    val exp = lic.expiresAt ?: 0L
    if (exp > 0 && now >= exp) return encryptRebindResponse(
        session,
        DeviceRebindPlainResponse(ok = false, error = "LICENSE_EXPIRED", expiresAt = exp)
    )

    val ownerId = lic.userId ?: return encryptRebindResponse(
        session,
        DeviceRebindPlainResponse(ok = false, error = "LICENSE_NOT_CLAIMED", expiresAt = exp)
    )
    val username = normalizeUsername(plain.username) ?: return encryptRebindResponse(
        session,
        DeviceRebindPlainResponse(ok = false, error = "BAD_REQUEST", expiresAt = exp)
    )
    val password = plain.password.takeIf { it.isNotBlank() } ?: return encryptRebindResponse(
        session,
        DeviceRebindPlainResponse(ok = false, error = "BAD_REQUEST", expiresAt = exp)
    )
    val user = state.store.findUserByUsername(username) ?: return encryptRebindResponse(
        session,
        DeviceRebindPlainResponse(ok = false, error = "USER_NOT_FOUND", expiresAt = exp)
    )
    if (!state.store.verifyUserPassword(user, password)) return encryptRebindResponse(
        session,
        DeviceRebindPlainResponse(ok = false, error = "BAD_CREDENTIALS", expiresAt = exp)
    )
    if (user.id != ownerId) return encryptRebindResponse(
        session,
        DeviceRebindPlainResponse(ok = false, error = "LICENSE_NOT_OWNED", expiresAt = exp)
    )

    state.store.upsertBinding(key, normalizeDeviceId(newDeviceId), iterations = 200_000)
    return encryptRebindResponse(session, DeviceRebindPlainResponse(ok = true, expiresAt = exp))
}
...
private fun encryptBindResponse(session: SecureSession, resp: DeviceBindPlainResponse): EncryptedResponse {
    val aad = ("sid=" + session.sid).toByteArray(StandardCharsets.UTF_8)
    val pt = JSON.encodeToString(resp).toByteArray(StandardCharsets.UTF_8)
    val (iv, ct) = aesGcmEncrypt(session.key32, pt, aad)
    return EncryptedResponse(iv = B64.encodeToString(iv), ct = B64.encodeToString(ct))
...
private fun encryptUnbindResponse(session: SecureSession, resp: DeviceUnbindPlainResponse): EncryptedResponse {
    val aad = ("sid=" + session.sid).toByteArray(StandardCharsets.UTF_8)
    val pt = JSON.encodeToString(resp).toByteArray(StandardCharsets.UTF_8)
    val (iv, ct) = aesGcmEncrypt(session.key32, pt, aad)
    return EncryptedResponse(iv = B64.encodeToString(iv), ct = B64.encodeToString(ct))
...
private fun encryptRebindResponse(session: SecureSession, resp: DeviceRebindPlainResponse): EncryptedResponse {
    val aad = ("sid=" + session.sid).toByteArray(StandardCharsets.UTF_8)
    val pt = JSON.encodeToString(resp).toByteArray(StandardCharsets.UTF_8)
    val (iv, ct) = aesGcmEncrypt(session.key32, pt, aad)
    return EncryptedResponse(iv = B64.encodeToString(iv), ct = B64.encodeToString(ct))
